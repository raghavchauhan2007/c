 ...
 * @param totalBytes pointer to variable totalbytes which stores the total bytes read here
 *
 * @return SUCCESS if file was read, table was generated and function returned cleanly
 * */
...
/**
 * Recursively builds the huffman codes for the characters in the frequency table
 * via traversing the tree, assigning 0 when taking the left branch
 * and 1 when taking the right branch
 *
 ...
 *
 * Internal nodes are written as 0 bit followed by the left subtree and the right subtree
 *
 ...
 *
 *  The original file size which helps the decoder to know after how many bytes it has to stop
 *
